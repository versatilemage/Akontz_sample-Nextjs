...
const CoursesDetailsSidebar = ({
	...
	useEffect(() => {
		if(enroled_courses?.length > 0){
			const ifPresent = enroled_courses.filter(filterPurchasedCourses)
			return (ifPresent?.length > 0 ? setAlreadyBuy(true):setAlreadyBuy(false))
		}
	}, [enroled_courses])
